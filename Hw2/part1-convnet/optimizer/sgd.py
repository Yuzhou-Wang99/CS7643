    ...
    def update(self, model):
        ...
        for idx, m in enumerate(model.modules):
            if hasattr(m, 'weight'):
                ...
                #############################################################################
                self.grad_tracker[idx]['dw'] *= self.momentum
                self.grad_tracker[idx]['dw'] -= self.learning_rate * m.dw
            ...
            if hasattr(m, 'bias'):
                ...
                #############################################################################
                self.grad_tracker[idx]['db'] *= self.momentum
                self.grad_tracker[idx]['db'] -= self.learning_rate * m.db
